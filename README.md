...
S’invocarà mitjançant el frontal síncron de la PCI. <br/>A continuació es detalla el diagrama de seqüència de la modalitat de consum: 
...
S’invocarà mitjançant el frontal asíncron de la PCI. <br/> A continuació es detalla el diagrama de seqüència de la modalitat de consum: 
<p align="center">
...
A continuació es detalla la missatgeria corresponent a les modalitats de consum del DCOC. <br/>Tot i que en la missatgeria genèrica de la PCI, no apareguin com a camps obligatoris, pel negoci que
...
Opcionalment es pot indicar l’identificador d’un col·legiat. 
...
Tot i que en la missatgeria genèrica de la PCI, no apareguin com a camps obligatoris, pel negoci que
implementa el DCOC, son obligatoris els següents elements de la petició: 
| _Element_ | _Descripció_ |
| --- | --- |
| Peticion/Atributos/Funcionario/NifFuncionario | Nif del funcionari que envia la petició.  |
...
camp de la pròpia resposta. Opcionalment es pot indicar l’identificador d’un col·legiat.
...
### 3.2.3 Petició – – dades específiques <a name="3.2.3"></a>
<p align="center">
<img align="center" src="respostaDescarregaDadesEspecífiques.png" />
</p>

| **Missatge xml  <br/> (P)etició/(R)esposta/(S)ubelement** | **Nom paràmetre (P) <br/> atribut(A)** | **Oblig.** | **Comentaris** |
| --- | --- | --- | --- |
|respostaDCOC (R)|idColegi(P)||Identificador del col·legi al qual pertany el projecte que es vol sol·licitar. El valor correspon a l’acrònim (en majúscules) del col·legi. Ex: COAC. |
||idColegiat(P)||Identificador del col·legiat del projecte.|
||projecte (P)||Subelement de tipus projecte.|
||error (P)|X|Subelement de tipus error.   |
|error (S)|codi (P) |X|Codi de l’error. En cas de resposta correcta el valor serà 0.  |
||descripcio (P) ||Descripció de l’error.|
|projecte (S) |idProjecte (P)||Identificador del projecte sol·licitat. |
||codiTipusProjecte (P)||Codi del tipus de projecte. Només vindrà informat en el cas en que l’emissor ho informi. Ex: 2 |
||arxius (P) ||Indica el tipus de projecte. Només vindrà informat en el cas en que l’emissor ho informi. Ex: Projecte de tipus Bàsic.|
||estatVisat (P)|X|Subelement de tipus arxius. |
...
||descripcio (P)||Descripció de l’arxiu que es retorna. Si l’arxiu es un certificat d’idoneïtat, el contigut d’aquest camp serà CERTIFICAT IDONEÏTAT. |
||tamany (P)||Grandària de l’arxiu (en bytes). |
||contentType (P) ||MimeType del arxiu. Ex: text/XML...|
||url (P)||Path relatiu al fitxer per descarregar. |
||contingut||Certificat en format XML, si s’escau|
||signaturaColegi(P)||Subelement de tipus arxiu. Com a màxim hi haurà 1 signatura que correspon a la del col·legi que visa el document. |
||signaturaProfessional (P)||Subelement de tipus arxiu. Poden venir N signatures, Correspon a la signatura del document per part d’un professional(qui visa el projecte) |
...
<p align="center">
<img align="center" src="respostaDadesTècniquesDadesEspecífiques.png" />
</p>
